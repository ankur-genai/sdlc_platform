...
# Upper bound on the serialized approved-requirements payload injected into the
# BA prompt. Beyond this the payload is truncated with a marker (see
# build_ba_prompt) to avoid overflowing the model's context window.
_MAX_REQUIREMENTS_CHARS = 60_000
...
def build_ba_prompt(approved_requirements: RequirementAgentOutput | dict | str) -> str:
    ...
    # Guard against an oversized requirements payload blowing the model's
    # context window. Serialize, and if the JSON is very large, truncate it with
    # a clear marker rather than silently sending a request that will be
    # rejected or truncated mid-token by the provider.
    serialized = (
        payload if isinstance(payload, str) else json.dumps(payload, indent=2)
    )
    if len(serialized) > _MAX_REQUIREMENTS_CHARS:
        serialized = (
            serialized[:_MAX_REQUIREMENTS_CHARS]
            + "\n\n...[requirements truncated for length; summarize the remainder"
