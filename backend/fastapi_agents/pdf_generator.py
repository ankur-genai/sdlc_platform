...
def generate_brd_pdf(project_id: int, db: Session) -> BytesIO:
    """
    Generates a consulting-grade 21-section Business Requirements Document (BRD) PDF
    (EY / Deloitte style) reading directly from the project's persisted Business Analyst
    Workspace database artifacts as the Single Source of Truth.
    """
    ...
    no_info = Paragraph("<font color='#64748B'>No information available for this project.</font>", styles_map['body'])
    ...
    raw_sections = [
        # 1. Executive Summary
        ("1.0 Executive Summary", lambda: [
            Paragraph("<b>Executive Briefing & Strategic Overview</b>", styles_map['h2']),
            Paragraph(raw_exec.get("overview", "Executive briefing defining platform requirements.") if isinstance(raw_exec, dict) else str(raw_exec), styles_map['body']),
            Spacer(1, 8),
            Table([
                [Paragraph("<b>Business Problem</b>", styles_map['header']), Paragraph("<b>Proposed Solution</b>", styles_map['header'])],
                [Paragraph(raw_exec.get("business_problem", "Operational friction"), styles_map['cell']), Paragraph(raw_exec.get("proposed_solution", "Automated platform"), styles_map['cell'])],
                [Paragraph("<b>Business Benefits</b>", styles_map['header']), Paragraph("<b>Expected ROI & Value</b>", styles_map['header'])],
                [Paragraph(raw_exec.get("business_benefits", "Reduced cycle time"), styles_map['cell']), Paragraph(raw_exec.get("expected_roi", "High ROI"), styles_map['cell'])],
            ], colWidths=[250, 254], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('BACKGROUND', (0,2), (-1,2), COLOR_SECONDARY), ('TEXTCOLOR', (0,2), (-1,2), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if raw_exec else [no_info]),

        # 2. Problem Statement
        ("2.0 Problem Statement", lambda: [
            Paragraph("<b>Current Business Context & Challenges</b>", styles_map['h2']),
            Spacer(1, 4),
            Table([
                [Paragraph("<b>Current State</b>", styles_map['header']), Paragraph(raw_prob.get("current_state", "Legacy manual operations"), styles_map['cell'])],
                [Paragraph("<b>Pain Points</b>", styles_map['header']), Paragraph(", ".join(raw_prob.get("pain_points", [])) if isinstance(raw_prob.get("pain_points"), list) else str(raw_prob.get("pain_points", "")), styles_map['cell'])],
                [Paragraph("<b>Business Need</b>", styles_map['header']), Paragraph(raw_prob.get("business_need", "Automated orchestration"), styles_map['cell'])],
                [Paragraph("<b>Desired Future State</b>", styles_map['header']), Paragraph(raw_prob.get("desired_future_state", "Cloud-native platform"), styles_map['cell'])],
                [Paragraph("<b>Business Value</b>", styles_map['header']), Paragraph(raw_prob.get("business_value", "60% operational reduction"), styles_map['cell'])],
            ], colWidths=[150, 354], style=[('BACKGROUND', (0,0), (0,-1), COLOR_LIGHT_BG), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if raw_prob else [no_info]),

        # 3. Business Objectives
        ("3.0 Business Objectives", lambda: [
            Table([
            ...
            ], colWidths=[200, 70, 134, 100], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if objectives else [no_info]),

        # 4. Scope & Boundaries
        ("4.0 Scope & Boundaries", lambda: [
            Paragraph("<b>In Scope:</b>", styles_map['h2']),
            *[Paragraph(f"• {item}", styles_map['bullet']) for item in (raw_scope.get("in_scope", []) if isinstance(raw_scope, dict) else [])],
            Spacer(1, 10),
            Paragraph("<b>Out of Scope:</b>", styles_map['h2']),
            *[Paragraph(f"• {item}", styles_map['bullet']) for item in (raw_scope.get("out_of_scope", []) if isinstance(raw_scope, dict) else [])]
        ] if raw_scope else [no_info]),

        # 5. Stakeholder Register
        ("5.0 Stakeholder Register", lambda: [
            Table([
                [Paragraph("<b>Stakeholder</b>", styles_map['header']), Paragraph("<b>Role</b>", styles_map['header']), Paragraph("<b>Responsibility</b>", styles_map['header']), Paragraph("<b>Decision Authority</b>", styles_map['header'])],
            ...
            ], colWidths=[120, 110, 184, 90], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if stakeholders else [no_info]),

        # 6. Personas & User Profiles
        ("6.0 Personas & User Profiles", lambda: [
            ...
            *[Table([
                ...
                [Paragraph(f"<b>Technical Skill:</b> {p.get('technical_skill', 'Intermediate') if isinstance(p, dict) else 'Intermediate'}", styles_map['cell']), Paragraph(f"<b>Primary Use Cases:</b> {p.get('use_cases', 'Workflow automation & dashboards') if isinstance(p, dict) else 'Workflow execution'}", styles_map['cell'])]
            ], colWidths=[250, 254], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)]) for p in (personas[:6] if isinstance(personas, list) else [])]
        ] if personas else [no_info]),

        # 7. Epics Breakdown
        ("7.0 Epics Breakdown", lambda: [
            Table([
            ...
            ], colWidths=[65, 179, 130, 65, 65], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if epics else [no_info]),

        # 8. Detailed User Stories Catalog
        ("8.0 Detailed User Stories Catalog", lambda: [
            Table([
            ...
            ], colWidths=[65, 60, 80, 209, 90], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if stories else [no_info]),

        # 9. Acceptance Criteria & Gherkin Scenarios
        ("9.0 Acceptance Criteria & Gherkin Scenarios", lambda: [
            Paragraph("<b>Formal BDD Acceptance Scenarios per Story</b>", styles_map['h2']),
            Spacer(1, 6),
            *[Paragraph(f"<b>{s.get('id', 'US-001')} — {s.get('title', 'Story Acceptance Criteria')}:</b><br/>" + "<br/>".join([f"• <font color='#1E293B'>{ac}</font>" for ac in s.get("acceptance_criteria", [])]), styles_map['body']) for s in (stories if isinstance(stories, list) else [])]
        ] if stories else [no_info]),

        # 10. Functional Requirements
        ("10.0 Functional Requirements", lambda: [
            Table([
            ...
            ], colWidths=[65, 219, 70, 80, 70], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if func_reqs else [no_info]),

        # 11. Non-Functional Requirements
        ("11.0 Non-Functional Requirements", lambda: [
            Table([
            ...
            ], colWidths=[70, 90, 264, 80], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if nonfunc_reqs else [no_info]),

        # 12. Business Rules Catalog
        ("12.0 Business Rules Catalog", lambda: [
            Table([
            ...
            ], colWidths=[70, 274, 80, 80], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if rules else [no_info]),

        # 13. Process Flows & Workflows
        ("13.0 Process Flows & Workflows", lambda: [
            ...
            *[Table([
            ...
            ], colWidths=[250, 254], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)]) for f in (process_flows if isinstance(process_flows, list) else [])]
        ] if process_flows else [no_info]),

        # 14. Risks & Success Metrics
        ("14.0 Risks & Success Metrics", lambda: [
            ...
            ], colWidths=[65, 179, 70, 60, 130], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)]),
            Spacer(1, 12),
            ...
            ], colWidths=[140, 70, 80, 134, 80], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if (risks or metrics) else [no_info]),

        # 15. Assumptions Catalog
        ("15.0 Assumptions Catalog", lambda: [
            Table([
            ...
            ], colWidths=[65, 349, 90], style=[('BACKGROUND', (0,0), (-1,0), COLOR_SECONDARY), ('TEXTCOLOR', (0,0), (-1,0), COLOR_WHITE), ('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('PADDING', (0,0), (-1,-1), 6)])
        ] if assumptions else [no_info]),

        # 16. Dependencies Register
        ("16.0 Dependencies Register", lambda: [
            Table([
        ...
        ] if dependencies else [no_info]),

        # 17. Requirements Traceability Matrix
        ("17.0 Requirements Traceability Matrix", lambda: [
            Table([
        ...
        ] if traceability else [no_info]),

        # 18. Revision History
        ("18.0 Revision History", lambda: [
            Table([
        ...
        ] if revisions else [no_info]),

        # 19. Approval & Sign-Off Matrix
        ("19.0 Approval & Sign-Off Matrix", lambda: [
            Table([
            ...
            ], colWidths=[110, 110, 80, 80, 124], style=[('GRID', (0,0), (-1,-1), 0.5, COLOR_BORDER), ('BACKGROUND', (0,0), (-1,0), COLOR_LIGHT_BG), ('PADDING', (0,0), (-1,-1), 6)])
        ] if approvals else [no_info])
    ]
    ...
    for sec_idx, (sec_title, content_builder) in enumerate(raw_sections, 1):
        dots = ". " * 28
        toc_rows.append([
            Paragraph(f"<b>{sec_title}</b>", styles_map['cell']),
            Paragraph(f"<font color='#94A3B8'>{dots}</font>", styles_map['cell']),
